...
function getPostsByTitleOrURL(db, params) {
  ...
  return new Promise((resolve, reject) => {
    db.query(
      `select id, title, description, url, likes, comments, nextCommentId, numComments, hasCommentSection from posts where isApproved = 1 and (title like ? or url like ?)`,
      [searchString, searchString],
      (err, result) => {
        if (err) {
          reject(err);
...
function getPostById(db, params) {
  ...
  return new Promise((resolve, reject) => {
    db.query(
      "select id, title, description, url, likes, comments, nextCommentId, numComments, hasCommentSection from posts where id = ?",
      [postId],
      (err, result) => {
        if (err) {
          reject(err);
...
function getPostsSorted(db, params) {
  ...
  const query =
    "select id, title, description, url, likes, comments, nextCommentId, numComments, hasCommentSection from posts where isApproved = 1 order by " +
    "case ? " +
